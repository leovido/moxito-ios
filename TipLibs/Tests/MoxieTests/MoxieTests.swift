...
final class MoxieClientTests: XCTestCase {
	...
	func testModel() async throws {
		...
		XCTAssertEqual(model.socials.first?.farcasterScore?.farRank, 215)
	}
	...
	func testModelWithMissingValues() async throws {
		...
		XCTAssertEqual(model.socials.first?.farcasterScore?.powerBoost, 0)
		XCTAssertEqual(model.socials.first?.farcasterScore?.liquidityBoost, 0)
	}
	...
	func testModelNil() async throws {
		...
		XCTAssertEqual(model.socials.isEmpty, true)
	}
}
	...
	"socials": [
		{
			...
			"farcasterScore": {
				...
				"liquidityBoost": 5.6524316965,
				"powerBoost": 33.067129972122935,
				"tvl": "1432338103548656334660223",
				"tvlBoost": 2.055564875175,
				"farScoreRaw": "4118623951883.293224688186170842"
			},
			"connectedAddresses": [
				{
